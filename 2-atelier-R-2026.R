...
# ATTENTION: le traitement des étiquettes de valeurs (SPSS...) ne correspond pas au fonctionnement de base de R. Il peut être préférable d'utiliser des packages qui prévoient l'utilisation de vecteurs labellisés (de classe labelled, voir cheat sheet labelled): Haven ou Labelled
# Mais après importation, ces variables restent numériques (avec métadonnées de classe spéciale), ie ne sont pas considérées comme catégorielles. 
# R s'attend à un format factor pour la majorité des fonctions d'analyses, graphiques... Il faudra donc éventuellement transformer ces vecteurs en facteur (to_factor() )
...
## Importer une base de données dans un module chargé particulier - Extrait de l'enquête "Histoire de vie" de l'Insee - https://www.insee.fr/fr/statistiques/2532244
data(hdv2003)
...
ncol(bd) # nombre de variables ou length(bd)
...
bd[ ,"relig"] # Sélectionner une variable (contenu textuel entre guillemets simples ou doubles), ou simplement bd["relig"]
...
bd$sexe[10:20] # Sélectionner les observations de 10 à 20 par position de la variable sexe - notez que la virgule n'est plus nécessaire puisque l'on applique notre condition à un élément qui n'a qu'une dimension, ie une variable
...
bd$age[is.na(bd$age)] <- mean(bd$age, na.rm=TRUE) # Attribuer une valeur quelconque aux valeurs manquantes d'une variable 
...
# L'opérateur d'indexation [] est utilisé à la droite de la flèche pour sélectionner et créer de nouveaux objets, MAIS il peut aussi être utilisé à la gauche de la <- pour modifier les éléments sélectionnés.
...
names(bd)[c(9,17)] <- c("classe", "bricolage") # On renomme les variables 9 et 17
...
quantile(bd$heures.tv, na.rm=TRUE) # argument probs= pour changer les proportions
...
# Pour un format de présentation, plusieurs packages sont disponibles comme gtsummary, flextable, summarytools,... 

# ex 1. : library(gtsummary)
# table <- tbl_cross(data = bd,row = trav.satisf,col = sexe,percent = "row")
# table

# ex 2. : library(flextable)
# as_flex_table(table)

# ex 3. : library(summarytools)
# view(ctable(bd$relig, bd$sexe, prop = "c", chisq = TRUE))

# ex 4. (avec grammaire tidyverse)
# library(flextable)
# library(officer)
# tbl_summary(data = bd, include = relig, statistic = all_categorical() ~ "{n} ({p}%)") |> as_flex_table() |> save_as_docx(path = "relig_table.docx")
...
freq(bd$qualif, cumul = TRUE, totals = FALSE, sort = "inc", round.digits = 0, report.nas = TRUE, missing = "ND", justify = "left")

## cumul: afficher ou non les %totals 
## totals: ajouter les effectifs totaux
## sort: trier le tableau par fréquence croissante (sort="inc") ou décroissante (sort="dec")
## round.digits: arrondir
## report.nas: exclure valeurs manquantes
## justify:alignement
## missing: texte indiquant les valeurs manquantes
...
# Fonction xtabs - Notation formule
...
## Pour enregistrer le script .R, faire simplement ctrl + s ou File + Save as...
...
save.image("fichier.RData")	# Sauvegarde tous les objets (tout ce qui se trouve dans l'onglet Environnement - pas le code et l'historique)
