...
# ou require() qui peut être utilisé dans des commandes if() pour vérifier au préalable la présence de packages installés
...
# Note: Certains packages fournissent des interfaces graphiques, disponibles entre autres via les Addins.
...
# La liste des arguments est ordonnée. Mais lorsque les arguments sont nommés, il n'est pas nécessaire de respecter l'ordre indiqué: mean(x, trim = 0, na.rm = FALSE, ...)
...
#  Tableau de données pouvant regrouper des vecteurs de différents types (variables numériques et/ou textuelles) puisque c'est en fait une liste.

# Tous les vecteurs d'un tableau doivent avoir la même longeur et posséder un nom unique.
...
# NB. Si l'on travaille avec les packages Tidyverse, les tableaux seront de format tibbles, une variante des dataframes qui permet des manipulations/opérations plus complexes. Il est possible de convertir un format à l'autre : as_tibble(hdv2003), as.data.frame() 